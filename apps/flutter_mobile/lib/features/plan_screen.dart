...
class _PlanScreenState extends State<PlanScreen> {
  ...
  _GrowthRule _ruleForMode(String mode, PlanSummary plan) {
    ...
    if (raw is Map<String, dynamic>) {
      return _GrowthRule(
        intervalDays: _clamp((raw['intervalDays'] as num?)?.toInt() ?? plan.growthIntervalDays, 1, 365),
        increment: _clamp((raw['increment'] as num?)?.toInt() ?? plan.growthIncrement, 0, 100),
      );
    }
    return _GrowthRule(intervalDays: plan.growthIntervalDays, increment: plan.growthIncrement);
  }

  int _parseController(TextEditingController controller, int fallback, int min, int max) {
  ...
  Map<String, dynamic> _buildPlanInput(PlanSummary plan) {
    ...
    for (final mode in _growthModes) {
      rulesByMode[mode] = {
        'intervalDays': _parseController(_perModeIntervalControllers[mode]!, sharedInterval, 1, 365),
        'increment': _parseController(_perModeIncrementControllers[mode]!, sharedIncrement, 0, 100),
      };
    }
    return <String, dynamic>{
      ...
      'rootAffixPerDay': _parseController(_rootAffixController, plan.rootAffixPerDay ?? 0, 0, 50),
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (applyToday == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('计划已保存，将从后续学习中生效')),
        );
      }
    } catch (error) {
      if (!mounted) return;
                        ...
                        _SectionCard(
                          ...
                          child: Column(
                            children: [
                              ...
                              _StepperField(
                                label: '词根词缀',
                                helper: '${int.tryParse(_rootAffixController.text) ?? (plan.rootAffixPerDay ?? 0)} 题',
                              ...
                              if (_growthRuleMode == 'perMode') ...[
                                ...
                                for (final mode in _growthModes) ...[
                                  _RuleEditorCard(
                                    ...
                                    intervalController: _perModeIntervalControllers[mode]!,
                                    incrementController: _perModeIncrementControllers[mode]!,
                                  ),
                                  const SizedBox(height: 12),
                                ],
                              ],
